...
namespace ClientAccountApp
{
    public sealed partial class LegacyWorkspacePage : Page
    {
        ...
        private int _clientViewMode = 0; // 0 = обычный, 1 = компактный, 2 = таблица
        ...
        private string GetCurrentClientViewModeText()
        {
            return _clientViewMode switch
            {
                1 => "Компактный",
                2 => "Табличный",
                _ => "Обычный"
        ...
        private static int ParseClientViewMode(string? viewMode)
        {
        ...
        private void RefreshSelectedClientHighlight()
        {
            ...
            DispatcherQueue.TryEnqueue(() =>
            {
                ...
                _highlightedClientBorder.BorderThickness = _clientViewMode == 2
                    ? new Thickness(1)
                    : new Thickness(2);
            });
        }
        ...
        {
            if (_highlightedClientBorder == null)
                return;
        ...
        {
            if (ClientsListView?.ContainerFromItem(client) is not ListViewItem container)
                return null;
        ...
        private static Border? FindClientCardBorder(DependencyObject root)
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                if (child is Border border &&
                    string.Equals(border.Tag?.ToString(), "ClientCardRoot", StringComparison.Ordinal))
                {
                    return border;
                }
        ...
        private void SetClientTypeComboBox(string clientType)
        {
            foreach (var item in ClientTypeComboBox.Items)
            {
                ...
                {
                    string itemText = comboBoxItem.Content?.ToString() ?? "";

                    if (string.Equals(itemText, clientType, StringComparison.OrdinalIgnoreCase))
                    {
        ...
        public LegacyWorkspacePage()
        {
            ...
            if (ClientFileFilterComboBox != null)
            {
                ClientFileFilterComboBox.SelectedIndex = 0;
            }
            ...
            var openClientFolderMenuItem = new MenuFlyoutItem
            {
                Text = "Открыть папку клиента"
            };
            _createContractMenuItem = new MenuFlyoutItem
            {
            ...
            _markContractSignedMenuItem = new MenuFlyoutItem
            {
                Text = "Отметить договор подписан",
                IsEnabled = false
            };
            ...
            var openFileMenuItem = new MenuFlyoutItem
            {
                Text = "Открыть файл"
            };
            ...
            var revealFileMenuItem = new MenuFlyoutItem
            {
            ...
            var openClientFolderFromFileMenuItem = new MenuFlyoutItem
            {
                Text = "Открыть папку клиента"
            };
            ...
            var convertToPdfMenuItem = new MenuFlyoutItem
            {
            ...
            var deleteFileMenuItem = new MenuFlyoutItem
            {
                Text = "Удалить файл"
            };
        ...
        {
            if (_createContractMenuItem == null || _markContractSignedMenuItem == null)
                return;
            ...
            try
            {
                ...
                bool hasGeneratedContract =
                    contract.GeneratedAt.HasValue ||
                    !string.IsNullOrWhiteSpace(contract.DocumentRelativePath);
        ...
        private void MarkContractSignedMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    StatusTextBlock.Text = "Не удалось найти клиента в базе данных.";
                    return;
                }
                ...
                {
                    StatusTextBlock.Text = "Сначала сформируйте договор для этого клиента.";
                    return;
                }

                if (string.Equals(contract.Status, "Договор подписан", StringComparison.OrdinalIgnoreCase))
                {
                    StatusTextBlock.Text = $"У клиента «{clientFromDb.Name}» договор уже отмечен как подписанный по организации «{organization.Name}».";
                    return;
                }
                ...
                StatusTextBlock.Text = $"Договор клиента «{clientFromDb.Name}» отмечен как подписанный по организации «{organization.Name}».";
            }
            catch (Exception ex)
            {
                StatusTextBlock.Text = $"Ошибка отметки договора: {ex.Message}";
            }
        }
        private void CreateContractMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(contract.DocumentRelativePath))
                {
                    ...
                    if (File.Exists(existingFullPath))
                    {
                        ...
                        StatusTextBlock.Text = $"Открыт уже сформированный договор клиента «{clientFromDb.Name}» по организации «{organization.Name}».";
                        return;
                    }
                }
                ...
                if (ClientsListView.SelectedItem is ClientInfo selectedClient &&
                    selectedClient.Id == clientFromDb.Id)
                {
                ...
                StatusTextBlock.Text = $"Договор сформирован по организации «{organization.Name}» и добавлен в файлы клиента «{clientFromDb.Name}».";
            ...
            {
                StatusTextBlock.Text = $"Ошибка формирования договора: {ex.Message}";
            }
        }
        private void ClientFileCard_RightTapped(object sender, RightTappedRoutedEventArgs e)
        {
            if (sender is not FrameworkElement element || element.DataContext is not ClientFile clientFile)
                return;
        ...
        private async void BankBicTextBox_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (result == null)
                {
                    StatusTextBlock.Text = $"Банк по БИК {bic} не найден.";
                    return;
                }
            ...
            {
                StatusTextBlock.Text = $"Ошибка поиска банка по БИК: {ex.Message}";
            }
        }
        ...
        private void OpenClientFileMenuItem_Click(object sender, RoutedEventArgs e)
        {
            OpenSelectedClientFileButton_Click(sender, e);
        }

        private void RevealClientFileMenuItem_Click(object sender, RoutedEventArgs e)
        {
            RevealSelectedClientFileButton_Click(sender, e);
        }

        private void OpenClientFolderFromFileMenuItem_Click(object sender, RoutedEventArgs e)
        {
            OpenClientFilesFolderButton_Click(sender, e);
        }

        private void ConvertClientFileToPdfMenuItem_Click(object sender, RoutedEventArgs e)
        {
            ConvertSelectedClientFileToPdfButton_Click(sender, e);
        }

        private void DeleteClientFileMenuItem_Click(object sender, RoutedEventArgs e)
        {
            DeleteSelectedClientFileButton_Click(sender, e);
        }
        ...
        {
            if (_rightTappedClientBorder == null)
                return;
            ...
            _rightTappedClientBorder.BorderThickness = _clientViewMode == 2
                ? new Thickness(0, 0, 0, 1)
                : new Thickness(1);
        ...
        {
            if (sender is not FrameworkElement element || element.DataContext is not ClientInfo client)
                return;
        ...
        {
            if (!_pendingClientIdFromDashboard.HasValue)
                return;

            if (ClientsListView == null || _clients.Count == 0)
                return;
        ...
        private void LoadFilesForSelectedClient()
        {
            ...
            using (var db = new AppDbContext())
            {
                var filesQuery = db.ClientFiles
                    .AsNoTracking()
                    .Where(f => f.ClientInfoId == selectedClient.Id);

                if (selectedCategoryFilter != "Все файлы")
                {
                    filesQuery = filesQuery.Where(f => f.Category == selectedCategoryFilter);
                }

                var filesFromDb = filesQuery
                ...
                if (!string.IsNullOrWhiteSpace(searchText))
                {
                    filesFromDb = filesFromDb
                        .Where(f =>
                            ...
                            (f.Category?.ToLowerInvariant().Contains(searchText) ?? false))
                        .ToList();
                }
        ...
        private void ClearClientSearchButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (SignatureFilterComboBox != null)
                SignatureFilterComboBox.SelectedIndex = 0;
        ...
        private void SetClientStatusComboBox(string status)
        {
            foreach (var item in ClientStatusComboBox.Items)
            {
                if (item is ComboBoxItem comboBoxItem)
                {
                    ...
                    if (string.Equals(itemText, status, StringComparison.OrdinalIgnoreCase))
                    {
        ...
        private void UpdateClientFormLabels()
        {
            ...
            if (DirectorLabelTextBlock == null || OgrnLabelTextBlock == null || OgrnTextBox == null)
            {
                return;
            }
            ...
            DirectorLabelTextBlock.Text = isEntrepreneur
                ? "ФИО предпринимателя"
                : "ФИО руководителя организации";

            OgrnLabelTextBlock.Text = isEntrepreneur
                ? "ОГРНИП"
                : "ОГРН";

            OgrnTextBox.PlaceholderText = isEntrepreneur
                ? "Введите ОГРНИП"
                : "Введите ОГРН";
        }
        ...
        private bool IsInnValidForCurrentType(string inn, string clientType)
        {
            ...
            return IsEntrepreneurType(clientType)
                ? inn.Length == 12
                : inn.Length == 10;
        }
        ...
        private bool ValidateClientForm()
        {
            ...
            if (ClientTypeComboBox == null ||
                ClientNameTextBox == null ||
                DirectorTextBox == null ||
                InnTextBox == null ||
                OgrnTextBox == null ||
                AddressTextBox == null ||
                ClientFormValidationTextBlock == null ||
                SaveClientButton == null ||
                UpdateSelectedClientButton == null ||
                FillByInnButton == null)
            {
                return false;
            }
            ...
            {
                errors.Add("Укажи наименование организации / ФИО.");
            }

            if (string.IsNullOrWhiteSpace(inn))
            {
                errors.Add("Укажи ИНН.");
            }
            ...
            {
                errors.Add("Укажи юридический адрес.");
            }

            bool isValid = errors.Count == 0;

            ClientFormValidationTextBlock.Text = isValid
        ...
        {
            if (ClientFileSearchTextBox == null || ClientFilesListView == null)
            {
                return;
            }
        ...
        {
            if (ClientFileSearchTextBox != null)
            {
                ClientFileSearchTextBox.Text = "";
            }
        ...
        {
            if (ClientFileFilterComboBox == null || ClientFilesListView == null)
            {
                return;
            }
        ...
        private void ClearClientFileInputFields()
        {
        ...
        private void RevealSelectedClientFileButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                StatusTextBlock.Text = "Сначала выбери файл, который нужно показать в проводнике.";
                return;
            }
            ...
            try
            {
                ...
                {
                    StatusTextBlock.Text = "Файл не найден на диске.";
                    return;
                }
            ...
            {
                StatusTextBlock.Text = $"Ошибка открытия файла в проводнике: {ex.Message}";
            }
        }
        ...
        private void UpdateFileActionButtonsState()
        {
            ...
            SetFileActionState(
                AddClientFileTopButton,
                AddClientFileTopButtonHost,
                hasClient,
                "Прикрепить файл к выбранному клиенту",
                "Сначала выберите клиента");

            SetFileActionState(
                OpenClientFilesFolderTopButton,
                OpenClientFilesFolderTopButtonHost,
                hasClient,
                "Открыть папку файлов выбранного клиента",
                "Сначала выберите клиента");

            SetFileActionState(
                OpenSelectedClientFileTopButton,
                OpenSelectedClientFileTopButtonHost,
                hasFile,
                "Открыть выбранный файл",
                "Сначала выберите файл");

            SetFileActionState(
                RevealSelectedClientFileTopButton,
                RevealSelectedClientFileTopButtonHost,
                hasFile,
                "Показать выбранный файл в папке",
                "Сначала выберите файл");

            SetFileActionState(
                ConvertSelectedClientFileToPdfTopButton,
                ConvertSelectedClientFileToPdfTopButtonHost,
                hasFile,
                "Конвертировать выбранный файл в PDF",
                "Сначала выберите файл");

            SetFileActionState(
                DeleteSelectedClientFileTopButton,
                DeleteSelectedClientFileTopButtonHost,
                hasFile,
                "Удалить выбранный файл",
                "Сначала выберите файл");
        }

        private void SetFileActionState(
            Button? button,
            ...
            string disabledToolTip)
        {
            ...
            if (host != null)
                ToolTipService.SetToolTip(host, isEnabled ? enabledToolTip : disabledToolTip);
        }
        private async Task LoadSelectedClientFilePreviewAsync(ClientFile clientFile)
        {
            ...
            try
            {
                ...
                {
                    await ShowImagePreviewAsync(fullPath);
                    FilePreviewStatusTextBlock.Text = $"Предпросмотр изображения: {clientFile.OriginalFileName}";
                    return;
                }

                if (extension == ".pdf")
                {
                    ShowPdfPreview(fullPath);
                    FilePreviewStatusTextBlock.Text = $"Предпросмотр PDF: {clientFile.OriginalFileName}";
                    return;
                }

                if (IsTextPreviewExtension(extension))
                {
                    await ShowTextPreviewAsync(fullPath);
                    FilePreviewStatusTextBlock.Text = $"Предпросмотр текста: {clientFile.OriginalFileName}";
                    return;
                }

                ShowFallbackPreview(
                    "Предпросмотр пока недоступен",
                    $"Тип файла: {clientFile.FileTypeDescription}. Его можно открыть внешней программой или позже конвертировать в PDF.");
                FilePreviewStatusTextBlock.Text = $"Нет встроенного предпросмотра для файла: {clientFile.OriginalFileName}";
            }
            catch (Exception ex)
            {
                ShowFallbackPreview("Ошибка предпросмотра", ex.Message);
                FilePreviewStatusTextBlock.Text = $"Ошибка предпросмотра: {clientFile.OriginalFileName}";
            }
        }

        private void ClearFilePreview()
        {
            FilePreviewImage.Source = null;
            FilePreviewImage.Visibility = Visibility.Collapsed;

            FilePreviewWebView.Source = null;
            FilePreviewWebView.Visibility = Visibility.Collapsed;

            FilePreviewTextBox.Text = "";
            FilePreviewTextBox.Visibility = Visibility.Collapsed;
        ...
        private async Task ShowTextPreviewAsync(string fullPath)
        {
            ...
            {
                text = text.Substring(0, maxLength) + Environment.NewLine + Environment.NewLine + "... [текст обрезан]";
            }
        ...
        private bool IsTextPreviewExtension(string extension)
        {
            return extension is ".txt" or ".xml" or ".csv" or ".json" or ".log";
        }
        private void OpenClientFilesFolderButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, папку файлов которого нужно открыть.";
                return;
            }

            try
            {
                ClientFileStorageService.OpenClientFolder(selectedClient);
                StatusTextBlock.Text = $"Открыта папка файлов клиента «{selectedClient.Name}».";
            }
            catch (Exception ex)
            {
                StatusTextBlock.Text = $"Ошибка открытия папки файлов клиента: {ex.Message}";
            }
        ...
        private void ConvertSelectedClientFileToPdfButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента.";
                return;
            }

            if (ClientFilesListView.SelectedItem is not ClientFile selectedFile)
            {
                StatusTextBlock.Text = "Сначала выбери файл, который нужно конвертировать в PDF.";
                return;
            }
            ...
            try
            {
                ...
                {
                    StatusTextBlock.Text = "Исходный файл не найден на диске.";
                    return;
                }

                string originalNameWithoutExtension = Path.GetFileNameWithoutExtension(selectedFile.OriginalFileName);
                var newPdfPath = ClientFileStorageService.GetNewPdfPathForClient(
    selectedClient,
    originalNameWithoutExtension + "_pdf");
                ...
                using (var db = new AppDbContext())
                {
                    db.ClientFiles.Add(newPdfFile);
                    db.SaveChanges();
                }

                LoadFilesForSelectedClient();
                StatusTextBlock.Text = $"Файл «{selectedFile.OriginalFileName}» конвертирован в PDF и прикреплен к клиенту.";
            }
            catch (Exception ex)
            {
                StatusTextBlock.Text = $"Ошибка конвертации в PDF: {ex.Message}";
            }
        }
        private string GetClientFileSearchText()
        {
            return ClientFileSearchTextBox?.Text?.Trim() ?? "";
        }
        ...
        private async void AddClientFileButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, которому нужно прикрепить файл.";
                return;
            }
            ...
            if (pickedFile == null)
            {
                StatusTextBlock.Text = "Выбор файла отменен.";
                return;
            }

            if (string.IsNullOrWhiteSpace(pickedFile.Path) || !File.Exists(pickedFile.Path))
            {
                StatusTextBlock.Text = "Не удалось получить путь к выбранному файлу.";
                return;
            }
            ...
            LoadFilesForSelectedClient();
            ClearClientFileInputFields();
            StatusTextBlock.Text = $"Файл «{clientFile.OriginalFileName}» категории «{clientFile.Category}» прикреплен к клиенту «{selectedClient.Name}».";
        }
        ...
        private void OpenSelectedClientFileButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientFilesListView.SelectedItem is not ClientFile selectedFile)
            {
                StatusTextBlock.Text = "Сначала выбери файл в списке.";
                return;
            }
        ...
        private void DeleteSelectedClientFileButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientFilesListView.SelectedItem is not ClientFile selectedFile)
            {
                StatusTextBlock.Text = "Сначала выбери файл, который нужно удалить.";
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                if (fileFromDb == null)
                {
                    StatusTextBlock.Text = "Не удалось найти файл в базе данных.";
                    return;
                }
            ...
            {
                ClientFileStorageService.DeleteClientFolderIfEmpty(currentClient);
            }
    ...
    Dictionary<int, ClientContract> contractMap)
        {
            ...
            {
                client.ContractStatus = string.IsNullOrWhiteSpace(contract.Status)
                    ? "Требует договора"
                    : contract.Status;
        ...
        private void LoadClientsFromDatabase(int? selectedClientId = null)
        {
            ...
            using (var db = new AppDbContext())
            {
                ...
                foreach (var client in clientsFromDb)
                {
                    ...
                    var clientSignatures = db.DigitalSignatures
                        .AsNoTracking()
                        .Where(s => s.ClientInfoId == client.Id)
                        .OrderBy(s => s.ExpiresDate)
                        .ToList();

                    FillClientSignatureInfo(client, clientSignatures);

                    var latestNote = db.ClientNotes
                        .AsNoTracking()
                        .Where(n => n.ClientInfoId == client.Id)
                        .OrderByDescending(n => n.CreatedAt)
                        .FirstOrDefault();

                    if (latestNote != null)
                    {
                        client.LatestNoteCreatedAt = latestNote.CreatedAt;
                    }

                    var primaryBank = db.BankAccounts
                        .AsNoTracking()
                        .Where(a => a.ClientInfoId == client.Id)
                        .OrderBy(a => a.BankName)
                        .FirstOrDefault();

                    if (primaryBank != null)
                    {
                        client.PrimaryBankName = primaryBank.BankName;
                    }

                    ApplyActiveOrganizationContractStateToClient(client, contractMap);

                    // ВАЖНО: фильтр применяем только после того, как ЭЦП уже заполнена
                    if (!MatchesCurrentFilter(client))
                        continue;
            ...
            if (selectedClientId.HasValue)
            {
                ...
                {
                    ClientsListView.SelectedItem = clientToSelect;
                    ClientsListView.ScrollIntoView(clientToSelect);
                }
            }

            if (_clients.Count == 0)
            {
                StatusTextBlock.Text = "По текущему фильтру клиенты не найдены.";
            }
        }
        ...
        private void FillClientSignatureInfo(ClientInfo client, List<DigitalSignature> signatures)
        {
            ...
            {
                client.NearestSignatureExpiresDate = null;
                client.SignatureWarningText = "ЭЦП не добавлена";
                return;
            }

            var nearestSignature = signatures
                .OrderBy(s => s.ExpiresDate)
                .First();
            ...
            if (daysLeft < 0)
            {
                client.SignatureWarningText = $"ПРОСРОЧЕНО: до {nearestSignature.ExpiresDate:dd.MM.yyyy}";
            }
            else if (daysLeft <= 7)
            {
                client.SignatureWarningText = $"СРОЧНО: истекает через {daysLeft} дн.";
            }
            else if (daysLeft <= 30)
            {
                client.SignatureWarningText = $"ВНИМАНИЕ: истекает через {daysLeft} дн.";
            }
            else
            {
                client.SignatureWarningText = $"Норма: действует до {nearestSignature.ExpiresDate:dd.MM.yyyy}";
            }
        }

        private void ShowClientsSectionButton_Click(object sender, RoutedEventArgs e)
        {
            ClientsSectionGrid.Visibility = Visibility.Visible;
        }

        private void ShowProblemSignaturesSectionButton_Click(object sender, RoutedEventArgs e)
        {
        ...
        private void LoadSignaturesForSelectedClient()
        {
            ...
            using (var db = new AppDbContext())
            {
                var signaturesFromDb = db.DigitalSignatures
                    .AsNoTracking()
                    .Where(s => s.ClientInfoId == selectedClient.Id)
                    .OrderBy(s => s.ExpiresDate)
                    .ToList();

                foreach (var signature in signaturesFromDb)
                {
        ...
        private void LoadBankAccountsForSelectedClient()
        {
            ...
            using (var db = new AppDbContext())
            {
                var accountsFromDb = db.BankAccounts
                    .AsNoTracking()
                    .Where(a => a.ClientInfoId == selectedClient.Id)
                    .OrderBy(a => a.BankName)
        ...
        private void LoadNotesForSelectedClient()
        {
            ...
            using (var db = new AppDbContext())
            {
                var notesFromDb = db.ClientNotes
                    .AsNoTracking()
                    .Where(n => n.ClientInfoId == selectedClient.Id)
                    .OrderByDescending(n => n.CreatedAt)
                    .ToList();
        ...
        private void SignatureFilterComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (SignatureFilterComboBox == null || ClientsListView == null)
                return;
        ...
        private void StatusFilterComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (StatusFilterComboBox == null || ClientsListView == null)
                return;
        ...
        private void SaveClientButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                StatusTextBlock.Text = "Исправь ошибки в форме перед сохранением.";
                return;
            }
        ...
        private void UpdateSelectedClientButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента в списке, которого нужно обновить.";
                return;
            }
            ...
            if (!ValidateClientForm())
            {
                StatusTextBlock.Text = "Исправь ошибки в форме перед обновлением.";
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                clientFromDb.ClientType = clientType;
                clientFromDb.Status = GetSelectedClientStatus();
                clientFromDb.Name = clientName;
                clientFromDb.DirectorFullName = directorName;
                clientFromDb.Inn = inn;
                clientFromDb.Ogrn = ogrn;
                clientFromDb.Address = address;

                db.SaveChanges();
            }

            LoadClientsFromDatabase(selectedClient.Id);
            StatusTextBlock.Text = $"Клиент «{clientName}» успешно обновлен.";
        }
        ...
        private void DeleteSelectedClientButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, которого нужно удалить.";
                return;
            }
            ...
            try
            {
                using (var db = new AppDbContext())
                {
                    ...
                    var clientAccounts = db.BankAccounts
                        .Where(a => a.ClientInfoId == selectedClient.Id)
                        .ToList();

                    var clientNotes = db.ClientNotes
                        .Where(n => n.ClientInfoId == selectedClient.Id)
                        .ToList();
                    ...
                    db.DigitalSignatures.RemoveRange(clientSignatures);
                    db.BankAccounts.RemoveRange(clientAccounts);
                    db.ClientNotes.RemoveRange(clientNotes);
        ...
        private void AddNoteButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, которому нужно добавить заметку.";
                return;
            }
            ...
            {
                StatusTextBlock.Text = "Введи текст заметки.";
                return;
            }
        ...
        {
            if (NotesListView.SelectedItem is not ClientNote selectedNote)
            {
                StatusTextBlock.Text = "Сначала выбери заметку, которую нужно удалить.";
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                if (noteFromDb == null)
                {
                    StatusTextBlock.Text = "Не удалось найти заметку в базе данных.";
                    return;
                }
        ...
        private void AddBankAccountButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, которому нужно добавить банковский счет.";
                return;
            }

            string bankName = BankNameTextBox.Text.Trim();
            string bic = BankBicTextBox.Text.Trim();
            string correspondentAccount = BankCorrespondentAccountTextBox.Text.Trim();
            string accountNumber = BankAccountNumberTextBox.Text.Trim();
            string comment = BankCommentTextBox.Text.Trim();

            if (string.IsNullOrWhiteSpace(bankName) || string.IsNullOrWhiteSpace(accountNumber))
            {
                StatusTextBlock.Text = "Укажи банк и расчетный счет.";
                return;
            }
        ...
        {
            if (AccountsListView.SelectedItem is not BankAccount selectedAccount)
            {
                StatusTextBlock.Text = "Сначала выбери счет в списке, который нужно удалить.";
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                {
                    StatusTextBlock.Text = "Не удалось найти счет в базе данных.";
                    return;
                }
        ...
        private void AddSignatureButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, которому нужно добавить ЭЦП.";
                return;
            }

            string authority = SignatureAuthorityTextBox.Text.Trim();
            string comment = SignatureCommentTextBox.Text.Trim();
            ...
            {
                StatusTextBlock.Text = "Укажи удостоверяющий центр.";
                return;
            }

            if (expiresDate < issuedDate)
            {
                StatusTextBlock.Text = "Дата окончания ЭЦП не может быть раньше даты получения.";
                return;
            }
        ...
        {
            if (SignaturesListView.SelectedItem is not DigitalSignature selectedSignature)
            {
                StatusTextBlock.Text = "Сначала выбери ЭЦП в списке, которую нужно удалить.";
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                {
                    StatusTextBlock.Text = "Не удалось найти ЭЦП в базе данных.";
                    return;
                }
        ...
        private void ClearFormButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            ClearInputFields();
            ClearClientCard();
            ClearSignatureSection();
            ClearBankAccountsSection();
            ClearNotesSection();
        ...
        {
            SignatureAuthorityTextBox.Text = "";
            SignatureCommentTextBox.Text = "";
        ...
        private void ClearBankAccountInputFields()
        {
            ...
            BankBicTextBox.Text = "";
            BankCorrespondentAccountTextBox.Text = "";
        ...
        private void ShowClientCard(ClientInfo client)
        {
            ...
            SelectedNearestSignatureTextBlock.Text = client.NearestSignatureExpiresDate.HasValue
                ? client.NearestSignatureExpiresDate.Value.ToString("dd.MM.yyyy")
                : "—";
            ...
            SelectedLatestNoteTextBlock.Text = client.LatestNoteCreatedAt.HasValue
                ? client.LatestNoteCreatedAt.Value.ToString("dd.MM.yyyy HH:mm")
                : "—";
        }
        ...
        private void UpdateSignatureSummary()
        {
            ...
            {
                SelectedNearestSignatureTextBlock.Text = "—";
                return;
            }

            var nearest = _signatures
                .OrderBy(s => s.ExpiresDate)
                .First();

            SelectedNearestSignatureTextBlock.Text = nearest.ExpiresDate.ToString("dd.MM.yyyy");
        }
        ...
        private void UpdateBankAccountSummary()
        {
            ...
            {
                SelectedPrimaryBankTextBlock.Text = "—";
                return;
            }

            SelectedPrimaryBankTextBlock.Text = _bankAccounts.First().BankName;
        }
        ...
        private void UpdateNoteSummary()
        {
            ...
            SelectedLatestNoteTextBlock.Text = latest.CreatedAt.ToString("dd.MM.yyyy HH:mm");
        }
        ...
        private void ClearClientCard()
        {
            ...
            SelectedStatusTextBlock.Text = "—";
            SelectedNameTextBlock.Text = "—";
            SelectedDirectorTextBlock.Text = "—";
            SelectedInnTextBlock.Text = "—";
            SelectedOgrnTextBlock.Text = "—";
            SelectedAddressTextBlock.Text = "—";
            SelectedSignatureCountTextBlock.Text = "—";
            SelectedNearestSignatureTextBlock.Text = "—";
            SelectedAccountCountTextBlock.Text = "—";
            SelectedPrimaryBankTextBlock.Text = "—";
        ...
        private void SearchTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            if (SearchTextBox == null || ClientsListView == null)
                return;
        ...
        private async void DownloadEgrulExtractButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                StatusTextBlock.Text = $"Ошибка получения выписки ФНС: {ex.Message}";
            }
        }
        private void ExportClientCardToWordButton_Click(object sender, RoutedEventArgs e)
        {
            if (ClientsListView.SelectedItem is not ClientInfo selectedClient)
            {
                StatusTextBlock.Text = "Сначала выбери клиента, карточку которого нужно выгрузить в Word.";
                return;
            }
            ...
            {
                StatusTextBlock.Text = $"Ошибка экспорта в Word: {ex.Message}";
            }
